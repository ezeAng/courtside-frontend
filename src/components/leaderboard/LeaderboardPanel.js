...
import { getOverallLeaderboard, getLeaderboard } from "../../services/api";
...
function LeaderboardPanel({
  ...
  compact = false,
  title = "Leaderboards",
  subtitle = "Track competitive rankings across formats.",
}) {
  ...
  const getSecondary = (entry) => entry?.gender;
  ...
  const topLabel = limit ? `TOP ${limit} PLAYERS ON COURTSIDE` : "TOP 100 PLAYERS ON COURTSIDE";
      ...
      >
        ...
        {onViewAll && (
          <Chip
            ...
            variant="outlined"
            onClick={onViewAll}
            sx={{ cursor: "pointer", ml: compact ? 2 : 0, mt: compact ? 0 : 1 }}
          />
        )}
      </Stack>
